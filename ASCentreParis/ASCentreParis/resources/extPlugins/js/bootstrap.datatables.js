(function (factory) {
	...
	$.extend( $.fn.dataTableExt.oPagination, {
		...
		"bs_full": {
			...
			// RCA  - Modification du style des boutons (hidden hide) - on ne presente que les boutons actifs
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
				{
					...
					$(nPaging).append(
						'<ul class="pagination">'+
						'<li class="hidden hide"><a  tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageFirst+'">'+'<i class="glyphicon glyphicon-fast-backward" title="glyphicon-fast-backward"></i>'+'</a></li>'+
						'<li class="hidden hide"><a  tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPagePrevious+'">'+'<i class="glyphicon glyphicon-step-backward" title="glyphicon-step-backward"></i>'+'</a></li>'+
						'<li><a tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageNext+'">'+'<i class="glyphicon glyphicon-step-forward" title="glyphicon-step-forward"></i>'+'</a></li>'+
						'<li><a tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageLast+'">'+'<i class="glyphicon glyphicon-fast-forward" title="glyphicon-fast-forward"></i>'+'</a></li>'+
						'</ul>'
					);
					var els = $('a', nPaging);
					var nFirst = els[0],
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
				{
					...
					for ( i=0, iLen=an.length ; i<iLen ; i++ )
					{
						...
						if ( oPaging.iPage === 0 ) {
							$('li:eq(0)', an[i]).addClass('hidden hide');
							$('li:eq(1)', an[i]).addClass('hidden hide');
						} else {
							$('li:eq(0)', an[i]).removeClass('hidden hide');
							$('li:eq(1)', an[i]).removeClass('hidden hide');
						}
						if ( oPaging.iPage === oPaging.iTotalPages-1 || oPaging.iTotalPages === 0 ) {
							$('li:eq(-1)', an[i]).addClass('hidden hide');
							$('li:eq(-2)', an[i]).addClass('hidden hide');
						} else {
							$('li:eq(-1)', an[i]).removeClass('hidden hide');
							$('li:eq(-2)', an[i]).removeClass('hidden hide');
						}
						$(sList)
